...
// PR-b hierarchy). A single row under the sidebar header: a leading rounded
// workspace glyph (the same initials as the rail square — they ARE the same
// workspace) + the REAL workspace name over a muted "Local workspace" sub-label,
// + a `chevron-down`. Clicking the pill opens the quick-switcher (Cmd+K palette),
// preserving the `open-switcher` test-id that previously lived in the header.
// (An earlier decorative "play/forward" glyph was removed on user feedback — it
// read as an interactive control but was a no-op.)
