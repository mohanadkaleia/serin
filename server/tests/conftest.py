"""Server test harness (ENG-63 D-6).
...
* **Per-test isolation = transaction rollback** — a connection begins an outer
  transaction, the ``AsyncSession`` is bound to it, and the transaction is
  rolled back after each test. No re-migrate, no truncate. App endpoints reach
  the same rolled-back session via ``app.dependency_overrides``.
* **In-process ASGI** via ``httpx.AsyncClient(ASGITransport(...))``.

Everything that needs the container is marked ``integration`` so a local dev
without Docker can run ``-m "not integration"``; CI (Docker present) runs the
full suite unfiltered.
"""
...
async def db_session(db_connection: AsyncConnection) -> AsyncIterator[AsyncSession]:
    ...
    maker = async_sessionmaker(bind=db_connection, expire_on_commit=False)
