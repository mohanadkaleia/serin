...
> **Live-WS caveat (ENG-92).** The "second browser sees the message live via WS fanout" leg passes **only under a forced `--ws wsproto` override** in the e2e harness. On the **default** self-host config (uvicorn + `websockets` v16) the WS upgrade is rejected with a 403: that backend does not surface `Sec-WebSocket-Protocol` in the ASGI scope, so the server can't read the `bearer, <token>` subprotocol and closes pre-accept (the same token authorizes `GET /v1/sync` 200 — only the WS handshake fails). Live WS on the default config is therefore **not yet proven** — tracked by **ENG-92** (pin the WS backend or the `websockets` version server-side). The golden path uses `--ws wsproto` until ENG-92 lands; **M2 live-sync is not exit-certified for the default config** until that fix ships and the override is removed. (Pull-based sync and reload-history — the other golden-path legs — are unaffected.)
